...
async def track_video(file_path):
    ...
    out = cv2.VideoWriter('%s_traced.avi' % (file_path.split('.')[0]), fourcc, cap.get(cv2.CAP_PROP_FPS), (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
    ...
    file_name = file_path.split('.')[0] + "_traced.avi"
    while cap.isOpened():
        ret, img = cap.read()
        if not ret:
            break
